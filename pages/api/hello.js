...
export default async function handler(req, res) {
	const txHistory = await fetch(
		"https://api.etherscan.io/api?module=account&action=txlist&address=0xf396b0385faC34992E96263256B1ffC02d21b5fc&startblock=0&endblock=99999999&page=1&offset=10&sort=asc&apikey=T5F14H1RXART3EQSAGZQSU8GJK9P1QRE4C"
	).then((res) => res.json());

	let result = txHistory.result.map(async (tx) => {
		const value = await getUSDValue(tx.timeStamp, tx.value);
		tx.usd = value[1];
		tx.eth = value[0];
