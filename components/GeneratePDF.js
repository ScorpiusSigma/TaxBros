...
export default function GeneratePDF() {
    ...
    const generate = async () => {
       ...
       const taxable = await fetch("/api/getTaxables", {
        ...
        doc.text('Date Range: ' + taxable['image'][taxable['image'].length - 1]['humantimeStamp'] + ' - ' + taxable['image'][0]['humantimeStamp'], 65, 20)
        autoTable(doc, {
        ...
        if ((inc - exp) <= 0){
            doc.text('Based on 21% Tax Charges you will be charged $0 in taxes',55,90 )
        }
        else{
            doc.text('Based on 21% Tax Charges you will be charged $' + ((inc - exp) * 0.21).toFixed(2) + ' in taxes',55,90 )
